...
func menu_tween_func(s : bool):
	...
	menu_tween.tween_property(AudioServer.get_bus_effect(MASTER_BUS, 0), "cutoff_hz", 500.0 if s else 20500.0, TWEEN_TIME)
	menu_tween.tween_property(AudioServer.get_bus_effect(MASTER_BUS, 1), "wet", 0.25 if s else 0.0, TWEEN_TIME)
	menu_tween.tween_method(func(v: float): AudioServer.set_bus_volume_db(MASTER_BUS, v), AudioServer.get_bus_volume_db(MASTER_BUS), -5.0 if s else 0.0, TWEEN_TIME)
